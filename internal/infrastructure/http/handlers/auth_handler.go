...
func (h *AuthHandler) Signup(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		AuditEmit(h.log, r, h.webhookEmitter, "user.signup", project.ID.String(), "", false, err.Error())
	...
	AuditEmit(h.log, r, h.webhookEmitter, "user.signup", project.ID.String(), result.User.ID.String(), true, "")
...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	if h.lockoutStore != nil {
		locked, retrySec := h.lockoutStore.IsLocked(r.Context(), project.ID.String(), email)
		if locked {
	...
	if err != nil {
		if h.lockoutStore != nil {
			h.lockoutStore.RecordFailure(r.Context(), project.ID.String(), email)
		}
		AuditEmit(h.log, r, h.webhookEmitter, "user.login", project.ID.String(), "", false, err.Error())
		middleware.RecordAuthAttempt("login", project.ID.String(), false)
		if err == domerrors.ErrInvalidCredentials {
			writeErr(w, http.StatusUnauthorized, ErrCodeInvalidCredentials, err.Error())
			return
		}
	...
	if h.lockoutStore != nil {
		h.lockoutStore.RecordSuccess(r.Context(), project.ID.String(), email)
	}
	AuditEmit(h.log, r, h.webhookEmitter, "user.login", project.ID.String(), result.User.ID.String(), true, "")
...
func (h *AuthHandler) Anonymous(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		AuditEmit(h.log, r, h.webhookEmitter, "auth.anonymous", project.ID.String(), "", false, err.Error())
		middleware.RecordAuthAttempt("anonymous", project.ID.String(), false)
		h.log.Error().Err(err).Msg("anonymous sign-in failed")
		writeErr(w, http.StatusInternalServerError, "", "internal error")
		return
	}
	AuditEmit(h.log, r, h.webhookEmitter, "auth.anonymous", project.ID.String(), result.User.ID.String(), true, "")
...
func (h *AuthHandler) Refresh(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		AuditEmit(h.log, r, h.webhookEmitter, "auth.refresh", "", "", false, err.Error())
		...
		if err == domerrors.ErrRefreshTokenReuse {
			...
			writeErr(w, http.StatusUnauthorized, ErrCodeSessionRevoked, domerrors.ErrInvalidToken.Error())
			return
		}
		if err == domerrors.ErrInvalidToken {
			writeErr(w, http.StatusUnauthorized, ErrCodeInvalidToken, err.Error())
			return
		}
	...
	AuditEmit(h.log, r, h.webhookEmitter, "auth.refresh", "", "", true, "")
...
func (h *AuthHandler) SendMagicLink(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		AuditEmit(h.log, r, h.webhookEmitter, "auth.magic_link.send", project.ID.String(), "", false, err.Error())
		middleware.RecordAuthAttempt("magic_link_send", project.ID.String(), false)
		h.log.Error().Err(err).Msg("send magic link failed")
		writeErr(w, http.StatusInternalServerError, "", "internal error")
		return
	}
	AuditEmit(h.log, r, h.webhookEmitter, "auth.magic_link.send", project.ID.String(), "", true, "")
